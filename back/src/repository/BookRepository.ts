...
export class BookRepository implements Book.Repository {
  ...
  async detachAllAuthors(book: BookEntity): Promise<void> {
    await getConnection().createQueryBuilder().relation(BookEntity, "authors").of(book).remove(book.authors);
  }

  async create(bookData: DeepPartial<BookEntity>): Promise<BookEntity> {
    const resultSave = await getConnection().getRepository(BookEntity).save(bookData);

    return await this.getByIdWithRelations(resultSave.id);
  }

  async delete(book: BookEntity): Promise<void> {
    await getConnection().getRepository(BookEntity).remove(book);
  }

  async getAll(): Promise<BookEntity[]> {
    return getConnection().getRepository(BookEntity).find();
  }
  ...
  async getById(bookId: string | number): Promise<BookEntity | undefined> {
    return getConnection().getRepository(BookEntity).findOne(bookId);
  }
  ...
  async hasExistIsbn(isbn: string): Promise<boolean> {
    return !!(await getConnection().getRepository(BookEntity).count({ isbn }));
  }
  ...
  async update(bookId: number | string, bookData: DeepPartial<BookEntity>): Promise<BookEntity> {
    await getConnection().getRepository(BookEntity).update(bookId, bookData);
