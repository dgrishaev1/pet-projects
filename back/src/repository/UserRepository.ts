...
export class UserRepository implements User.Repository {
  async create(userData: DeepPartial<UserEntity>): Promise<UserEntity> {
    return await getConnection().getRepository(UserEntity).save(userData);
  }

  async getById(userId: string | number): Promise<UserEntity | undefined> {
    return await getConnection().getRepository(UserEntity).findOne(userId);
  }

  async update(userId: string | number, userData: DeepPartial<UserEntity>): Promise<boolean> {
    return !!(await getConnection().getRepository(UserEntity).update(userId, userData))?.affected;
  }
  ...
  async hasExistEmail(email: string): Promise<boolean> {
    return !!(await getConnection().getRepository(UserEntity).count({ email }));
  }

  async hasExistLogin(login: string): Promise<boolean> {
    return !!(await getConnection().getRepository(UserEntity).count({ login }));
  }

  async delete(user: UserEntity): Promise<void> {
    await getConnection().getRepository(UserEntity).delete(user);
  }
}
