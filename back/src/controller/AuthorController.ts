...
import { schemaAuthorCreate, schemaAuthorList, schemaAuthorUpdate, schemaAuthorValidateId } from "../schema/author";
...
export const authorGetAll: App.Action<Author.List.Request> = async (req, res) => {
  await schemaAuthorList.validate(req.query);
...
export const authorDelete: App.Action<Author.Delete.Request> = async (req, res) => {
  await schemaAuthorValidateId.validate(req.params);
