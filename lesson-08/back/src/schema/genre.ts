...
export const schemaGenreList: Yup.SchemaOf<Genre.List.Query> = Yup.object().shape({
  search: Yup.string(),
});
...
export const schemaGenreValidateId: Yup.SchemaOf<Genre.Delete.Params | Genre.Single.Params> = Yup.object().shape({
  id: Yup.number().min(1).required(),
});
